...
export type ProcessingModel = "deep-filter-net-3" | "demucs";
...
export const useFileUpload = (
  ...
  const [state, setState] = useState<FileUploadState>({
    ...
    bitrate: "same-as-input",
    attenuationLimit: 100,
    selectedModel: "deep-filter-net-3",
  });
  ...
  const setBitrate = useCallback((bitrate: string) => {
    setState((prev) => ({ ...prev, bitrate }));
  }, []);

  const setAttenuationLimit = useCallback((attenuationLimit: number) => {
    setState((prev) => ({ ...prev, attenuationLimit }));
  }, []);

  const setSelectedModel = useCallback((model: ProcessingModel) => {
